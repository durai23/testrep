...
Opt:	-prefix		   < pattern >	output filename prefix (default: _IC)
	-erode	 	   < int >	erosion kernel size (default: 2)
...
${scripts}ConditionalDilation_clust.sh \
 ${TMP}${inputbName}_cbq_mask_${absthresh}.nii.gz \
 ${JacRank%.nii.gz}_IC+Fuzzy_C${csf}.nii.gz \
 $iter \
 $minsd $maxsd \
 $tolerance >> ${TMP}Debug.log 2>&1
 
before=`3dBrickStat -slow -non-zero -volume ${TMP}${inputbName}_cbq_mask_${absthresh}.nii.gz`
after=`3dBrickStat -slow -non-zero -volume ${TMP}${inputbName}_cbq_mask_${absthresh}_CD.nii.gz`
...
suffix=${TMP}${inputbName}_cbq_mask_${absthresh}_CD.nii.gz
...
mvV $suffix ${dest}/${pref}_mask.nii.gz

3dcalc \
 -prefix ${dest}/${pref}.nii.gz \
 -a $input \
 -b ${dest}/${pref}_mask.nii.gz \
 -expr "a*step(b)" \
 -verbose \
 -nscale >> ${TMP}Debug.log 2>&1
