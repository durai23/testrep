...
then
	...
	then
		l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_t1,${l} >> ${output}
	else	 
		count=1;	
		for j in `echo ${n}`
	       	do 
			l=`sed -n ${j}p ${dcmFile} `
			echo ${sub}0_t1_${count},${l} >> ${output}
			count=$(( count + 1 ))
	       	done
	fi
fi
...
then 
	...
	wc=`echo ${n} | wc -w`
        ...
        then
                l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_t2,${l} >> ${output}
        else
                count=1;
                for j in `echo ${n}`
                do
                        l=`sed -n ${j}p ${dcmFile} `
                        echo ${sub}0_t2_${count},${l} >> ${output}
                        count=$(( count + 1 ))
                done
        fi
...
then 
	...
	wc=`echo ${n} | wc -w`
        ...
        then
                l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_t1ce,${l} >> ${output}
        else
                count=1;
                for j in `echo ${n}`
                do
                        l=`sed -n ${j}p ${dcmFile} `
                        echo ${sub}0_t1ce_${count},${l} >> ${output}
                        count=$(( count + 1 ))
                done
        fi
...
then 
	...
	wc=`echo ${n} | wc -w`
        ...
        then
                l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_flair,${l} >> ${output}
        else
                count=1;
                for j in `echo ${n}`
                do
                        l=`sed -n ${j}p ${dcmFile} `
                        echo ${sub}0_flair_${count},${l} >> ${output}
                        count=$(( count + 1 ))
                done
        fi
...
#extract perf
n=`cat ${dcmFile} | cut -d, -f4 | grep -n -i "perf" | grep -E -v -i "rel|cbv|cbf|ttp|pbp" | cut -d: -f1`
if [ ! `echo ${n} | wc -l` -eq 0 ]
then 
	...
	wc=`echo ${n} | wc -w`
        ...
        then
                l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_perf,${l} >> ${output}
        else
                count=1;
                for j in `echo ${n}`
                do
                        l=`sed -n ${j}p ${dcmFile} `
                        echo ${sub}0_perf_${count},${l} >> ${output}
                        count=$(( count + 1 ))
                done
        fi
...
then 
	...
	wc=`echo ${n} | wc -w`
        ...
        then
                l=`sed -n ${n}p ${dcmFile} `
                echo ${sub}0_dti,${l} >> ${output}
        else
                count=1;
                for j in `echo ${n}`
                do
                        l=`sed -n ${j}p ${dcmFile} `
                        echo ${sub}0_dti_${count},${l} >> ${output}
                        count=$(( count + 1 ))
                done
        fi
