...
################################################ FUNCTIONS ################################################
import os, sys, time
sys.path.insert(0, '/cbica/projects/PENN_MS/Scripts/SkullStripCorrection') #python will search this folder before all others - even builtin ones
...
def usage():
  ...
  Optional:
    ...
    [-c --coreg     ]	< file  >	Output filename for the co-registered T2 image in T1 space (default: None)
    ...
    [-l --minsd     ]	< float >	Minimum std dev to accept during conditional dilation (default: -0.5)
...
try:
	opts, files = getopt.getopt( sys.argv[1:], "i:r:j:o:n:l:h:t:s:ap:c:v",
	  [ "T1input", "T2input", "jacrank", "out", "iter","minsd","maxsd", "tolerance", "str", "affine", "prob", "coreg", "verbose" ] )
...
for o, a in opts:
	...
	if o in [ "-r", "--T2input" ]:
		...
		coregdName, coregbName, coregExt = FileAtt( coreg )
...
T1_dat 	= T1_img.get_data().astype( 'float' )
T1_hdr 	= T1_img.get_header()

JR_dat 	= JR_img.get_data().astype( 'float' )
JR_hdr 	= JR_img.get_header()

JR_dat 	= JR_dat / JR_dat.max() 				# scaling the jacrank to [0,1]

T1_dat 	= T1_dat - T1_dat.min() 				# shifting the min to 0
T1_dat 	= T1_dat / T1_dat.max() * scaleFactor			# scaling the T1 image to [0,255]

T1_img 	= nib.Nifti1Image( T1_dat, None, T1_hdr)
T1_img.set_data_dtype( 'float' )
T1_img.to_filename( os.path.join( TMP + '/' + T1inputbName + T1inputExt ) )

if affine == int(1):
	T2_img 	= nib.load( os.path.join( T2 ) )
	T2_dat 	= T2_img.get_data().astype( 'float' )
	T2_hdr 	= T2_img.get_header()

	T2_dat 	= T2_dat - T2_dat.min() 				# shifting the min to 0
	T2_dat 	= T2_dat / T2_dat.max() * scaleFactor			# scaling the T1 image to [0,255]
...
if affine == int(0):
	...
	T2_dat_coreg = np.zeros_like( T1_dat ).astype( 'float' )
	...
	T2_img 	= nib.Nifti1Image( T2_dat, None, T2_hdr)
	T2_img.set_data_dtype( 'float' )
	T2_img.to_filename( os.path.join( TMP + '/' + T2inputbName + T2inputExt ) )
	
	for cost in costfuncs:
		cmd = 'flirt ' \
			'-in ' + os.path.join( TMP + '/' + T2inputbName + T2inputExt ) + ' ' \
			'-ref ' + os.path.join( TMP + '/' + T1inputbName + T1inputExt ) + ' ' \
		...
		T2_dat_coreg	= T2_dat_coreg + T2_img.get_data().astype( 'float' )
	...
	T2_dat_coreg = T2_dat_coreg / len( costfuncs )
	T2_dat 	     = T2_dat_coreg.copy()
	
	# Save co-registered T2 file
	if coreg:
		coreg_img = nib.Nifti1Image( T2_dat_coreg, None, T1_hdr)
		coreg_img.set_data_dtype( 'float' )
		coreg_img.to_filename(os.path.join( coregdName + '/' + coregbName + coregExt ))

	# Remove rescaled T2
	os.remove( TMP + '/' + T2inputbName + T2inputExt )
...
os.remove( TMP + '/' + T1inputbName + T1inputExt )
...
for tissue in 1, 2, 3:
	...
	# GM
		...
		T1_dat_prob 	= st.norm.sf( T1_dat_zscore ) * 4
		
		prob_final = T1_dat_prob

#		T2_dat_zscore 	= np.abs( ( T2_dat - T2_dat_thresh_res[ T2segimg == int(tissue) ].mean() ) / T2_dat_thresh_res[ T2segimg == int(tissue) ].std() )
#		T2_dat_prob 	= st.norm.sf( T2_dat_zscore ) * 4
#		
#		prob_final = T1_dat_prob * T2_dat_prob
...
#		T2_dat_zscore 	= np.abs( ( T2_dat - T2_dat_thresh_res[ T2segimg == int(tissue) ].mean() ) / T2_dat_thresh_res[ T2segimg == int(tissue) ].std() )
#		T2_dat_prob 	= st.norm.sf( T2_dat_zscore )
#		
#		prob_final = T1_dat_prob * T2_dat_prob
...
if probJR:
	...
	JR_dat_updated_final_img.to_filename(os.path.join( probJR + '_NonBrain.nii.gz' ))
...
sizes 			= sp.ndimage.sum( data_maskimg_updated, label_im, range(nb_labels + 1))
...
for tissue in 3, 4:
	...
	if tissue == 3:
		runawaycond = True if mean >= 2*stdev else False
	elif tissue == 4:
#		runawaycond = True if mean >= stdev else False
		runawaycond = True if mean >= 2*stdev else False
		
	while ( i <= numiter ) and ( volRat >= tolerance ) and runawaycond :
