...
# modiefied by Jan Schreiber (j.schreiber@fz-juelich.de)
# replacing the SGI / Condor job scheduler with MPI / SLURM
...
##############################
#
#  Start MPI/SLURM modification
#
##############################

# check if this is a JURECA node
SYSTEMNAME=""
...
if [[ -z $RANDOMISE_NUM_TASKS_PER_NODES ]]; then
    if [[ -e /etc/FZJ/systemname ]]; then
        RANDOMISE_NUM_TASKS_PER_NODES=24
    else
        RANDOMISE_NUM_TASKS_PER_NODES=$(grep -c ^processor /proc/cpuinfo)
    fi
fi
echo "Running $RANDOMISE_NUM_TASKS_PER_NODES task(s) per node."

if [[ -z $RANDOMISE_NUM_NODES ]]; then
    RANDOMISE_NUM_NODES=1
fi
NUM_TASKS=$(cat ${DIRNAME}/${BASENAME}.generate | wc -l)
MAX_NODES_REQUIRED=$(echo "$NUM_TASKS / $RANDOMISE_NUM_TASKS_PER_NODES + 1" | bc)
if [[ $RANDOMISE_NUM_NODES -gt $MAX_NODES_REQUIRED ]]; then
    echo "Reducing from $RANDOMISE_NUM_NODES to $MAX_NODES_REQUIRED node(s)."
    RANDOMISE_NUM_NODES=$MAX_NODES_REQUIRED
fi
echo "Using $RANDOMISE_NUM_NODES node(s)."

# submit the generate script
if [[ $IS_JURECA -eq 1 ]]; then
    MAIL_USER=$(q_cpuquota | grep 'E-Mail:' | head -n 1 | tr -s ' ' | cut -d ' ' -f 2)
    PARAMETERS="--mail-user=$MAIL_USER --mail-type=END --job-name=${BASENAME}.proc --nodes=$RANDOMISE_NUM_NODES --ntasks-per-node=$RANDOMISE_NUM_TASKS_PER_NODES --cpus-per-task=1 --time=23:00:00 --partition="batch" --output=${DIRNAME}/${BASENAME}_proc_SLURM_out.%j --error=${DIRNAME}/${BASENAME}_proc_SLURM_err.%j"
    GENERATE_ID=$(srun $PARAMETERS python fzj_mpi_parallel_simple.py ${DIRNAME}/${BASENAME}.generate)
    echo "JobID: $GENERATE_ID"
else
    mpiexec -n $RANDOMISE_NUM_TASKS_PER_NODES python fzj_mpi_parallel_simple.py "${DIRNAME}/${BASENAME}.generate"
fi

##############################
#
#  End MPI/SLURM modification
#
##############################
...
ACTIVESEED=1
while [ \$ACTIVESEED -le $SLOTS_PER_CONTRAST ] ; do
  echo "Removing temporary files for SEED $ACTIVESEED"
...
if [[ $IS_JURECA -eq 1 ]]; then
    ...
    PARAMETERS="--mail-user=$MAIL_USER --mail-type=END --job-name=${BASENAME}.postproc --nodes=1 --ntasks-per-node=1 --cpus-per-task=1 --time=01:00:00 --partition=batch --output=${DIRNAME}/${BASENAME}_postproc_SLURM_out.%j --error=${DIRNAME}/${BASENAME}_postproc_SLURM_err.%j"
    DEFRAGMENT_ID=$(srun $PARAMETERS ${DIRNAME}/${BASENAME}.defragment)
    echo "JobID postprocessing: $DEFRAGMENT_ID"
else
    /bin/bash "${DIRNAME}/${BASENAME}.defragment"
fi
