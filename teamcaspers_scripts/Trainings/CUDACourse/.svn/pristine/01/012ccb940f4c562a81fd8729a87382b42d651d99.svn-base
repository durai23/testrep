...
__global__ void sort(const T* data_in, T* data_out){
    ...
    typedef cub::BlockLoad<const T*, BLOCK_WIDTH, ITEMS_PER_THREAD, BLOCK_LOAD_ALGO> BlockLoadT;
...
int main(){
    ...
    // Sort data    
    sort<512, 8, cub::BLOCK_LOAD_TRANSPOSE, cub::BLOCK_STORE_TRANSPOSE><<<1, 512>>>(d_gpu, 
result_gpu);
