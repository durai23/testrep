...
void jacobi()
{
	while (residue > tol && iter < iter_max)
	{
		...
		if ( size == 2 )
		{
		    // TODO: Call CUDA-aware MPI on device buffers to update halo data instead of the manual staging through
			//       host buffers.
			MPI_Status status;
			if ( rank == 0)
			{
				CUDA_CALL( cudaMemcpy( sendBuffer, unew_d+(n-2)*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToHost ) );
				MPI_Sendrecv( sendBuffer, m-2, MPI_FLOAT, 1, 0, recvBuffer , m-2, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, &status );
				CUDA_CALL( cudaMemcpy( u_d+(n-1)*m+1, recvBuffer, (m-2)*sizeof(float), cudaMemcpyHostToDevice ) );
			}
			else
			{
				CUDA_CALL( cudaMemcpy( sendBuffer, unew_d + 1*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToHost ) );
				MPI_Sendrecv( sendBuffer, m-2, MPI_FLOAT, 0, 0, recvBuffer, m-2, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &status );
				CUDA_CALL( cudaMemcpy( u_d+0*m+1, recvBuffer, (m-2)*sizeof(float), cudaMemcpyHostToDevice ) );
			}
		}
#endif //USE_MPI
...
int init(int argc, char** argv)
{
   /* TODO: Initialize the CUDA device before MPI init with
    *       cudaSetDevice (Use the environment variable MPI_LOCALRANKID) to
    *       determine the rank before MPI_Init */
#ifdef USE_MPI
	MPI_Init(&argc,&argv);
